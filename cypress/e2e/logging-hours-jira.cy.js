...
describe("log work hours", () => {
  ...
  it("logs into Jira and applies the filter for a specific sprint", () => {
    ...
    // Prompt the user for their Jira name
    cy.window().then((win) => {
      const userName = win.prompt(
        "Please enter your Jira name exactly as it appears (e.g., 'Alberto Odio'):"
      );

      if (!userName || userName.trim() === "") {
        throw new Error(
          "Jira name was not provided. Please provide your exact Jira name."
        );
      }
        ...
        .then(($editor) => {
          ...
          cy.window().then((win) => {
            ...
            if (sprintValue) {
              ...
              const jqlQuery = `project = "GL" AND "qa owner[user picker (single user)]" = currentUser() AND sprint = ${sprintValue} AND status IN ("Done in sandbox", "Done in Stage", "PO validation") ORDER BY created DESC`;
